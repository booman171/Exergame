...
import com.mbientlab.metawear.tutorial.exergame.Dodge;
import com.mbientlab.metawear.tutorial.exergame.DodgeMain;
import com.mbientlab.metawear.tutorial.exergame.FieldView;
...
public class Field {
	...
	double goalRatioBullet = 0.0; // fraction of field containing start and end areas
	...
	public double goalHeightBullet() {
		return goalRatio*aspectRatio;
	}
	...
	Bullet newBullet() {
		...
		default: // bottom to top
			bullet.setPosition(new Vec2(p1, goalRatioBullet*aspectRatio));
			bullet.setTargetPosition(new Vec2(p2, (1-goalRatioBullet)*aspectRatio));
			break;
	...
	public void tick(double dt) {
		...
		if (dodger!=null) {
			...
			if(System.currentTimeMillis() - DodgeMain.startTime >= 20000 && newLevel == true){
			...
			//Log.e("serious", Double.toString(dpos.x*4500) + ", Neutral: " + Integer.toString(FieldView.screenWidth));

			if(dpos.x*4500 <= FieldView.screenWidth)
				inNeutralZone = true;
			else
				inNeutralZone =false;

            /**Check if Bullets hit Playship**/
			boolean hit = false;
			for(int i=0; i<maxBullets; i++) {
				Bullet b = this.bullets[i];
				if (b!=null) {
					if (dpos.squaredDistanceTo(b.getPosition()) < 0.025*0.025 && dpos.x*4500 > FieldView.screenWidth) {
						hit = true;
						bulletIndexesToRemove.add(i);
					}
				}
	...
	public void createDodger() {
		...
		dodger.setPosition(new Vec2(0.5, (movingUp) ? yoffset : aspectRatio-yoffset + 1));
		dodger.setSpeed(0.4);
